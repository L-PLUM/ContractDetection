...
sourceUnit
    : packageDefinition end ( importDefinition end )* ( contractDefinition end )*
    ;
...
importDefinition
    : 'import' ( importDeclaration | '(' ( importDeclaration end )* ')' )
    ;
...
constDefinition
    : 'const' ( constDeclaration | '(' ( constDeclaration end )* ')' )
    ;

//ConstDeclaration      = IdentifierList [ [ Type ] "=" ExpressionList ] .
constDeclaration
    : identifierList ( typeExpression? '=' expressionList )?
    ;
...
//TypeDefinition     = "type" ( TypeDeclaration | "(" { TypeDeclaration ";" } ")" ) .
typeDefinition
    : 'type' ( typeDeclaration | '(' ( typeDeclaration end )* ')' )
    ;
...
//FunctionDefinition = "func" FunctionName ( Function | Signature ) .
...
//MethodDefinition   = "func" Receiver MethodName ( Function | Signature ) .
//Receiver     = Parameters .
methodDefinition
...
//VarDefinition     = "var" ( VarDeclaration | "(" { VarDeclaration ";" } ")" ) .
//VarDeclaration     = IdentifierList ( Type [ "=" ExpressionList ] | "=" ExpressionList ) .
varDefinition
    : 'var' ( varDeclaration | '(' ( varDeclaration end )* ')' )
    ;

varDeclaration
    : identifierList ( typeExpression ( '=' expressionList )? | '=' expressionList )
    ;
...
//SimpleStatement = EmptyStatement | ExpressionStatement | SendStatement | IncDecStatement | Assignment | ShortVarDefinition .
simpleStatement
    : sendStatement
    | expressionStatement
    | incDecStatement
...
//Assignment = ExpressionList assignOperator ExpressionList .
assignment
    : expressionList assignOperator expressionList
    ;

//assignOperator = [ add_op | mul_op ] "=" .
assignOperator
    : ('+' | '-' | '|' | '^' | '*' | '/' | '%' | '<<' | '>>' | '&' | '&^')? '='
    ;


//ShortVarDefinition = IdentifierList ":=" ExpressionList .
shortVarDefinition
...
//SwitchStatement = ExpressionSwitchStatement | TypeSwitchStatement .
switchStatement
    : expressionSwitchStatement | typeSwitchStatement
    ;
...
//ExpressionSwitchCase = "case" ExpressionList | "default" .
expressionSwitchStatement
    : 'switch' ( simpleStatement ';' )? expression? '{' expressionCaseClause* '}'
    ;
...
//TypeSwitchStatement  = "switch" [ SimpleStatement ";" ] TypeSwitchGuard "{" { TypeCaseClause } "}" .
//TypeSwitchGuard = [ identifier ":=" ] PrimaryExpression "." "(" "typeExpression" ")" .
//TypeCaseClause  = TypeSwitchCase ":" StatementList .
//TypeSwitchCase  = "case" TypeLiteral | "default" .
//TypeLiteral        = Type { "," Type } .
typeSwitchStatement
    : 'switch' ( simpleStatement ';' )? typeSwitchGuard '{' typeCaseClause* '}'
    ;
typeSwitchGuard
    : ( identifier ':=' )? primaryExpression '.' '(' 'type' ')'
    ;
typeCaseClause
    : typeSwitchCase ':' statementList
    ;
typeSwitchCase
    : 'case' typeSwitchCaseList | 'default'
    ;
typeSwitchCaseList
...
//RecvStatement   = [ ExpressionList "=" | IdentifierList ":=" ] RecvExpr .
//RecvExpr   = Expression .
selectStatement
    : 'select' '{' commClause* '}'
    ;
...
//InterfaceType      = "interface" "{" { MethodDeclaration ";" } "}" .
//MethodDeclaration         = MethodName Signature | InterfaceTypeName .
//MethodName         = identifier .
//InterfaceTypeName  = TypeName .
interfaceType
    : 'interface' '{' ( methodDeclaration end )* '}'
    ;
...
//KeyType     = Type .
mapType
    : 'map' '[' typeExpression ']' elementType
    ;
...
parameterDeclaration
    : identifierList? '...'? typeExpression
    ;

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Operands

//Operand     = Literal | OperandName | MethodExpression | "(" Expression ")" .
//Literal     = BasicLiteral | CompositeLiteral | FunctionLiteral .
//BasicLiteral    = int_lit | float_lit | imaginary_lit | rune_lit | string_lit .
...
//Tag            = string_lit .
structType
    : 'struct' '{' ( fieldDeclaration end )* '}'
    ;

fieldDeclaration
    : ({noTerminatorBetween(2)}? identifierList typeExpression | anonymousField) STRING_LIT?
    ;
...
//	Operand |
//	Conversion |
//	PrimaryExpression selectorType |
//	PrimaryExpression Index |
//	PrimaryExpression Slice |
//	PrimaryExpression TypeAssertion |
//	PrimaryExpression Arguments .
//
//selectorType       = "." typeExpression .
//Index          = "[" Expression "]" .
...
arguments
    : '(' ( ( expressionList | typeExpression ( ',' expressionList )? ) '...'? ','? )? ')'
    ;
